...
public class DocumentObject implements Comparable<DocumentObject> {
	...
	public DocumentObject(int id, String year, String dcn, String rev,
		...
		this.groupCode = groupCode;
		this.title = truncate(title, DataAccessObject.TITLE_MAX_LENGTH);
		this.authors = truncate(authors, DataAccessObject.AUTHORS_MAX_LENGTH);
		this.uploadDate = mentorDateFormat.parse(uploadDate);
		this.url = new URL(url);
		fileName = this.url.getPath();
		fileName = truncate(new File(fileName).getName(), DataAccessObject.FILENAME_MAX_LENGTH);
		this.notes = notes;
	}
