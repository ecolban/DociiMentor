 ...
 * 
 * An instance scrapes information about each document on a page and saves it in
 * the database unless the information has already been captured.
 * 
					...
					if (m.matches()) {
						...
						if (isGoodGroup(groupCode)) {
							...
							UI.LOGGER
									.log(Level.WARNING,
											String.format(
													"File %s not added to the database. "
															+ "Group code %s is currently not in configuration file.\n",
													fileName, groupCode));
						}
					} else {
						patternIndex = 0;
